...
/**
 * \brief utn_getAlfanumerico: Solicita un nombre al usuario.
 * \param char* mensaje: Es el mensaje a ser mostrado al usuario.
 * \param char* mensaje: Es el mensaje de error a ser mostrado al usuario.
 * \param int* pResultado: Puntero al espacio de memoria donde se dejara el valor obtenido.
 * \param int reintentos: Cantidad de oportunidades para ingresar el dato.
...
int utn_getCuit(char msg[], char msgError[], char pResult[], int reintentos, int limit)
{
	...
	if(msg != NULL && msgError != NULL && pResult != NULL && reintentos >= 0)
	{
		...
		{
			printf("\n%s", msg);
			if(myGets(bufferString,LIMITE_BUFFER_STRING) == 0 && esCuit(bufferString) == 1 && strnlen(bufferString, sizeof(bufferString)) < limit)
			{
				retorno = 0;
...
/**
 * \brief esCuit:	Verifica una cadena recibida como parametro para determinir si es un cuit valido.
 * \param char* cadena: Cadena a analizar.
 * \param int limite: Indica la cantidad de letras maxima de la cadena.
